...
function createTree(arrName, vYeobu, checkValue) {
	...
	if (treeNodes.length > 0) {
		...
		// vHtmlCode +="<table width='181' height='94' border='2' align='center'
		// cellpadding='0' cellspacing='0'><tr>";
		// vHtmlCode +="<td valign='bottom'
		// background='/images/egovframework/com/left_menu_top.gif'
		...
		if (startNode != 0) {
			...
			// vHtmlCode +="<div class='LeftMenuTitle'><font color='#ffffff'>" +
			// nodeValues[2] + "</font></div></td></tr>"
		} else {
			// vHtmlCode +="<img src='"+imgpath+"menu_base.gif' border='0'
			// align='absbottom' alt='' >메뉴목록<br></td></tr>";
		}
		var recursedNodes = new Array();
		addTreeNode(startNode, recursedNodes);
		// vHtmlCode +="<tr><td height='30' valign='bottom'
		// background='/images/egovframework/com/left_menu_btm.gif'
		// style='background-repeat:no-repeat'>&nbsp;</td></tr></table>";
...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			if (hasChildNode) {
				vHtmlCode += vHtmlCodeBg + "<a href='#'>" + nodeValues[2]
						+ "</a></li>";
			} else {
				// Start link
