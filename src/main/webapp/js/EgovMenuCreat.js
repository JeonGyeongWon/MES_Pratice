...
function createTree(arrName) {
	...
	if (treeNodes.length > 0) {
		...
		if (startNode != 0) {
			...
			document
					.write("<input type='checkbox' name='checkAll' class='check2' onclick='javascript:fCheckAll();'>메뉴목록<br>");
...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			document.write("&nbsp;&nbsp;&nbsp;");
			document.write("<input type='checkbox' id='" + i
					+ "' name='checkField' class='check2' ");
			if (nodeValues[4] == 1) {
				document.write(" checked ");
			}
			document
					.write("onclick='javascript:fCheckDir(this.name, this.value,"
							+ i + ");' value=" + nodeValues[0] + ">");
			if (hasChildNode) {
				...
				document
						.write("<img id='icon"
								...
								+ "' src='"
								+ imgpath
								+ "menu_page.gif' border='0' align='absbottom' alt='Page'>");
