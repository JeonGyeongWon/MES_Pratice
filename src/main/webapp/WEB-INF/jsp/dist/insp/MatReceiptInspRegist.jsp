...
function setInitial() {
    // 입하일자
    calender($('#searchInspFrom, #searchInspTo, #searchTransFrom, #searchTransTo'));

    $('#searchInspFrom, #searchInspTo,#searchTransFrom, #searchTransTo').keyup(function (event) {
        if (event.keyCode != '8') {
            var v = this.value;
            if (v.length === 4) {
                this.value = v + "-";
...
function setValues() {
    ...
    fields["columns.1"] = [
        ...
        //     },
        {
            ...
            renderer: function (value, metaData, record, rowIndex, colIndex, store, view) {
                ...
                var url = "<c:url value='/dist/insp/MatReceiptInspRegistD.do?no=' />" + record.get("INSPECTIONPLANNO") + "&org=" + record.get("ORGID") + "&company=" + record.get("COMPANYID");
    ...
    items["api.1"] = {};
    $.extend(items["api.1"], {
        read: "<c:url value='/select/dist/insp/MatReceiptInspRegist.do' />"
    });

    items["api.2"] = {};
    $.extend(items["api.2"], {
        read: "<c:url value='/select/dist/insp/MatReceiptInspRegistDetail.do' />"
    });
...
function onMasterClick(dataview, record, item, index, e, eOpts) {
    var params = {
        ...
        INSPECTIONPLANNO: record.data.INSPECTIONPLANNO,
    };
                            ...
                            load: function (dataStore, rows, bool) {
                                ...
                                if (cnt > 0) {
                                    ...
                                    var params = {
                                        ...
                                        INSPECTIONPLANNO: model.data.INSPECTIONPLANNO,
                                    };
                                    extGridSearch(params, gridnms["store.2"]);
                                } else {
...
function setLovList() {
    ...
    $("#searchInspNo").bind("keydown", function (e) {
        ...
        source: function (request, response) {
            $.getJSON("<c:url value='/MatReceiptInspRegistInspNoListLov.do' />", {
