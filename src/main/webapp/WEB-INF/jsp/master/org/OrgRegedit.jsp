...
//         resizable : false,
    ...
    items["api.1"] = {};
    $.extend(items["api.1"],{
        create : "<c:url value='/insert/org/OrgRegedit.do' />"
    });
    $.extend(items["api.1"],{
        read : "<c:url value='/select/org/OrgRegedit.do' />" 
    });
    $.extend(items["api.1"],{
        update : "<c:url value='/update/org/OrgRegedit.do' />"
    });
    $.extend(items["api.1"],{
        destroy : "<c:url value='/delete/org/OrgRegedit.do' />" 
    });
...
function btnAdd1Click(o, e) {
 ...
 function btnSav1Click(o, e) {
    ...
    var check = model.get("ORGCODE")+"";
    if (check.length == 0) {
        extAlert("사업장 코드를 입력하세요.");
        count++;
    }
