...
$(document).ready(function () {
	...
	function setValues() {
	  ...
	  items["api.1"] = {};
	  $.extend(items["api.1"], {
	    read: "<c:url value='/select/prod/outtrans/OutTransManage.do' />"
	  });
	  $.extend(items["api.1"], {
	    destroy: "<c:url value='/delete/prod/outtrans/ProdOutTransManageD.do' />"
	  });
	              ...
	              if (count44 == 0) {
	                ...
	                for (var j = 0; j < count44; j++) {
	                  ...
	                  if (chk === true) {
	                    ...
	                    model.set("CARTYPE", model44.data.CARTYPE);
	                    model.set("CARTYPENAME", model44.data.CARTYPENAME);
	                    ...
	                    model.set("WORKORDERSEQ", model44.data.WORKORDERSEQ); // 작업지시순번
	...
	function setValues_Popup() {
	  ...
	  items["api.44"] = {};
	  $.extend(items["api.44"], {
	    read: "<c:url value='/ProdOutTransManagePop.do' />"
	  });
	          ...
	          listeners: {
	            refresh: function (dataView) {
	              Ext.each(dataView.panel.columns, function (column) {
	                if (column.dataIndex.indexOf('CON') >= 0) {
	                  column.autoSize();
	                  column.width += 5;
	                  if (column.width < 150) {
	                    column.width = 150;
                  ...
                  if (column.dataIndex.indexOf('ORDERNAME') >= 0 || column.dataIndex.indexOf('DRAWINGNO') >= 0 || column.dataIndex.indexOf('ITEMNAME') >= 0 || column.dataIndex.indexOf('CARTYPENAME') >= 0 || column.dataIndex.indexOf('ROUTINGNAME') >= 0) {
	                  column.autoSize();
	                  column.width += 5;
	                  if (column.width < 80) {
	                    column.width = 80;
	...
	function fn_list() {
	  go_url("<c:url value='/prod/outtrans/OutTransList.do'/>");
	}
	...
	function fn_save() {
	  ...
	  url1 = "",
	  msgGubun = 0;
	  var statusconfirm = $('#TransStatus').val();
	  var statuschk = true;
	  ...
	  if (isNew) {
	    url = "<c:url value='/insert/prod/outtrans/OutTransManage.do' />";
	    url1 = "<c:url value='/insert/prod/outtrans/OutTransManageGrid.do' />";
	    msgGubun = 1;
	  } else {
	    url = "<c:url value='/update/prod/outtrans/OutTransManage.do' />";
	    url1 = "<c:url value='/update/prod/outtrans/OutTransManageGrid.do' />";
	    msgGubun = 2;
	  }
	                ...
	                Ext.Ajax.request({
	                  ...
	                  success: function (conn, response, options, eOpts) {
	                    ...
	                    if (dataSuccess > 0) {
	                      ...
	                      go_url("<c:url value='/prod/outtrans/OutTransManage.do?no=' />" + outtransno + "&org=" + orgid + "&company=" + companyid);
	...
	function fn_delete() {
	  ...
	  url = "<c:url value='/delete/prod/outtrans/ProdOutTransManageH.do' />";
...
<body oncontextmenu="return false" onselectstart="return false" ondragstart="return false" style="height: auto;">
    ...
    <div id="wrap">
        ...
        <div id="gridPopupArea" style="width: 1500px; padding-top: 0px; float: left;"></div>
