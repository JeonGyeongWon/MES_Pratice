...
$(document).ready(function () {
	...
	function setValues() {
	  ...
	  gridnms["grid.1"] = "MonthlyEquipList";
	  ...
	  items["api.1"] = {};
	  $.extend(items["api.1"], {
	    read: "<c:url value='/select/prod/manage/MonthlyEquipList.do' />"
	  });
	...
	function setExtGrid() {
	  ...
	  Ext.define(gridnms["controller.1"], {
	    ...
	    refs: {
	      MonthlyEquipList: '#MonthlyEquipList',
	    },
	  ...
	  Ext.define(gridnms["panel.1"], {
	    ...
	    items: [{
	        ...
	        viewConfig: {
	          itemId: 'MonthlyEquipList',
	          striptRows: true,
	          forceFit: true,
	          listeners: {
	            refresh: function (dataView) {
	              Ext.each(dataView.panel.columns, function (column) {
	                if (column.dataIndex.indexOf('WORKDEPTNAME') >= 0) {
	                  column.autoSize();
	                  column.width += 5;
	                  if (column.width < 80) {
	                    column.width = 80;
	...
	function fn_search() {
	  ...
	  var companyid = $('#searchCompanyId option:selected').val();
	  var searchyear = $('#searchYear').val();
	  var searchWorkDept = $('#searchWorkDept').val();
	...
	function fn_excel_download() {
	  ...
	  go_url("<c:url value='/prod/manage/ExcelDownload.do?GUBUN='/>" + "MONTHLYEQUIP"
	     + "&ORGID=" + orgid + ""
	     + "&COMPANYID=" + companyid + ""
	     + "&SEARCHYEAR=" + searchyear
	     + "&WORKDEPT=" + searchWorkDept
	...
	function setLovList() {
	  ...
	  .autocomplete({
	    source: function (request, response) {
	      $.getJSON("<c:url value='/searchMonthlyEquipListLov.do' />", {
	        keyword1: extractLast(request.term),
	        ORGID: $('#searchOrgId option:selected').val(),
	        COMPANYID: $('#searchCompanyId option:selected').val(),
	        SEARCHYEAR: $('#searchYear').val(),
	        GUBUN: 'WORKDEPT',
	    ...
	    select: function (e, o) {
	      $("#searchWorkDeptName").val(o.item.label);
	      $("#searchWorkDept").val(o.item.value);
                            ...
                            <div>
                                ...
                                <table class="tbl_type_view" border="1">
                                    ...
                                    <tr style="height: 34px;">
                                        ...
                                        <td>
		                                        <input type="text" id="searchWorkDeptName" name="searchWorkDeptName" class="imetype input_center" onKeyUp="javascript:this.value=this.value.toUpperCase();" oncontextmenu="return false" style="width: 72%;" />
		                                        <input type="hidden" id="searchWorkDept" name="searchWorkDept" />
                                        </td>
