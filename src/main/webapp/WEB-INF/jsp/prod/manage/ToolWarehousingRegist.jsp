            ...
            listeners: {
                beforecheckchange: function (options, row, value, event) {
                    ...
                    if (value) {
                        var result = record.data.TRANSFINISHYN;
                        if (result == "Y") {
                            extAlert("입고완료된 경우 삭제가 불가능합니다.<br/>다시 한번 확인해주십시오.");
                            return false;
                        }
                    }
                }
            ...
            tbar: [
                ...
                '유형', {
                    ...
                    handler: function () {
                        ...
                        if (checknum == 0) {
                            extAlert("공구를 선택하지 않으셨습니다.<br/>다시 한번 확인해주십시오.");
                            return false;
                        }
...
//             $('input[name=searchCustomerName]').attr('disabled', false).removeClass('ui-state-disabled');
            ...
            $('#popupItemType').val('T');
            $('input[name=searchItemType]').val("공구");
            $('input[name=searchItemType]').attr('disabled', false).removeClass('ui-state-disabled');
        } else {
        ...
        extAlert("입고 등록 하실 경우에만 공구 불러오기가 가능합니다.");
        return;
    }
}
...
//                                     }
        ...
        extAlert("입고 등록 하실 경우에만 입고대기LIST(발주정보) 불러오기가 가능합니다.");
        return;
    }
}

function fn_popup_search1() { //입고대기 List
    global_popup_flag2 = false;
    var sparams = {
        ORGID: $('#popupOrgId').val(),
        COMPANYID: $('#popupCompanyId').val(),
...
function btnDel1() {
    ...
    var url = "<c:url value='/delete/prod/manage/ToolWarehousingDetailList.do' />";
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (deletecount == count) {
                                        extAlert(msgdata);
                                        go_url("<c:url value='/prod/manage/ToolWarehousingRegist.do?no=' />" + transno + "&org=" + orgid + "&company=" + companyid);
                                    }
                                },
                                error: ajaxError
                            });
                    ...
                    listeners: {
                        refresh: function (dataView) {
                            Ext.each(dataView.panel.columns, function (column) {
                                ...
                                if(column.dataIndex.indexOf('ITEMSTANDARD') >= 0){
                                	column.autoSize();
                                    column.width += 5;
                                    if (column.width < 80) {
                                        column.width = 80;
...
function setValues_Popup1() {
    ...
    items["api.44"] = {};
    $.extend(items["api.44"], {
        read: "<c:url value='/ToolWarehousingRegistPop1.do' />"
    });
...
function fn_save() {
    ...
    if (isNew) {
        url = "<c:url value='/insert/prod/manage/ToolWarehousingMasterList.do' />";
        url1 = "<c:url value='/insert/prod/manage/ToolWarehousingDetailList.do' />";
        msgGubun = 1;
    } else {
        url = "<c:url value='/update/prod/manage/ToolWarehousingMasterList.do' />";
        url1 = "<c:url value='/update/prod/manage/ToolWarehousingDetailList.do' />";
        msgGubun = 2;
    }
...
function fn_delete() {
    ...
    url = "<c:url value='/delete/prod/manage/ToolWarehousingMasterList.do' />";
...
function fn_list() {
    go_url("<c:url value='/prod/manage/ToolWarehousingManage.do'/>");
}
                        ...
                        <form id="master" name="master" action="" method="post">
                            ...
                            <table class="tbl_type_view" border="0">
                                ...
                                <tr style="height: 34px;">
                                    ...
                                    <td><select id="searchCompanyId" name="searchCompanyId" class="input_left " style="width: 50%; visibility: hidden;">
                                    ...
                                    <td>
                                        <div class="buttons" style="float: right; margin-top: 3px;">
                                            <a id="btnChk1" class="btn_search" href="#" onclick="javascript:btnSel2();"> 입고대기 LIST(발주정보) </a>
