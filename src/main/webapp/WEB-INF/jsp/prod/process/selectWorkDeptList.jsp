...
function fn_goNextPage( value ) {
    go_url('<c:url value="/prod/process/selectWorkOrderRegist.do?type="/>' + "${searchVO.type}" + "&gubun=" + value );
}
...
<!--             </span> -->
                ...
                <button type="button" class="red r shadow" onclick="fn_goHome();" style="width: 40%; height: 100px; font-size: 40px; font-weight: bold; margin-left: 15px; margin-top: 10px; margin-bottom: 5px;">이전화면&nbsp;( BACK )</button>
								<h3 class="shadow" style="height: 70px; font-size: 35px; background-color: orange; font-weight: bold; margin-top: 0px; text-align: center; display: -webkit-flex; display: flex; -webkit-align-items: center; align-items: center; -webkit-justify-content: center; justify-content: center;">해당 작업반을 선택하십시오.</h3>
						</center>
						<br />
						<div id="search_field" style="margin-bottom: 15px;">
								<fieldset>
										<legend>작업반 선택 영역</legend>
										<div align="center">
												<br>
												<c:forEach items="${selectWork}" var="selectWork">
												    <button type="button" class="blue2 h r shadow" onclick="fn_goNextPage('<c:out value="${selectWork.VALUE}"/>');" style="width: 16%; height: 100px; font-size: 35px; font-weight: bold; color: #fff; margin-left: 20px; margin-bottom: 20px;">
																<c:out value="${selectWork.LABEL}" />
														</button>
												</c:forEach>
										</div>
								</fieldset>
