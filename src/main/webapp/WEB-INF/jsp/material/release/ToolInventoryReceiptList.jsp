...
$(document).ready(function () {
	...
	function setValues_Inventory() {
	  ...
	  gridnms["grid.1"] = "ToolInventoryReceiptList";
	  ...
	  fields["columns.1"] = [
	    // Display Columns
	    {
	      ...
	      renderer: function (value, meta, record) {
	        ...
	        var inventoryqty = record.data.PRESENTINVENTORYQTY * 1;
	        if ((invsafeqty > inventoryqty) || (inventoryqty < 0)) {
	  ...
	  items["api.1"] = {};
	  $.extend(items["api.1"], {
	    read: "<c:url value='/select/material/release/ToolInventoryReceiptList.do' />"
	  });
	...
	function setExtGrid_Inventory() {
	  ...
	  Ext.define(gridnms["controller.1"], {
	    ...
	    refs: {
	      ToolInventoryList: '#ToolInventoryList',
	    },
	          ...
	          listeners: {
	            refresh: function (dataView) {
	              Ext.each(dataView.panel.columns, function (column) {
	                if (column.dataIndex.indexOf('ITEMNAME') >= 0 || column.dataIndex.indexOf('ITEMSTANDARD') >= 0) {
	                  column.autoSize();
	                  column.width += 5;
	                  if (column.width < 80) {
	                    column.width = 80;
	...
	function fn_excel_download() {
	  ...
	  go_url("<c:url value='/material/release/ExcelDownload.do?GUBUN='/>" + "TOOLINVENTORY"
	     + "&ORGID=" + orgid + ""
	     + "&COMPANYID=" + companyid + ""
	     + "&YYYYMM=" + searchMonth + ""
	     + "&ITEMCODE=" + searchItemCode + ""
	    ...
	    source: function (request, response) {
	      $.getJSON("<c:url value='/searchItemNameLov.do' />", {
	        ...
	        response($.map(data.data, function (m, i) {
	            return $.extend(m, {
	              label: m.ITEMNAME + ", " + m.ITEMSTANDARD,
	              value: m.ITEMCODE,
	              ITEMNAME: m.ITEMNAME,
				...
				<div id="container">
						...
						<div id="content">
                            ...
                            <input type="hidden" id="searchCustomerCode" name="searchCustomerCode" />
                                        ...
                                        <td>
                                          <input type="text" id="searchOrderName" name="searchOrderName" class="imetype  input_center" onKeyUp="javascript:this.value=this.value.toUpperCase();" oncontextmenu="return false"  style="width: 97%;" readonly />
                                        </td>
																		</tr>
                                    <tr style="height: 34px;">
