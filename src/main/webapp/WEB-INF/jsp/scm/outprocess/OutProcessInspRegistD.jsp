...
$(document).ready(function () {
	...
	function setInitial() {
	  ...
	  // 날짜
	  var inspno = "${searchVO.INSPECTIONPLANNO}";

	  if (inspno == "") {
	    $("#InspDate").val(getToDay("${searchVO.TODAY}") + "");
	  } else {
	    $("#InspDate").val(getToDay("${searchVO.InspDate}") + "");
	  }
	...
	function setValues() {
	  ...
	  gridnms["grid.1"] = "MatReceivingInspRegistD";
	  ...
	  $.extend(items["btns.ctr.1"], {
	    "#inspectionRegist": {
	      itemclick: 'onMyviewCheckresultClick'
	    }
	  });
	...
	function onMyviewCheckresultClick(dataview, record, item, index, e, eOpts) {
	...
	function setExtGrid() {
	  ...
	  Ext.define(gridnms["controller.1"], {
	    ...
	    btnDel1: btnDel1,
	    btnSel1: btnSel1,
	    onMyviewCheckresultClick: onMyviewCheckresultClick,
	  });
	...
	function setValues_Popup() {
	  ...
	  gridnms["grid.4"] = "Popup1"; // Popup
	  gridnms["grid.44"] = "Popup2"; // 입고검사 세부 항목 LIST   (복사할 Data 조회)
	  ...
	  fields["columns.4"] = [{
	      ...
	      widget: {
	        ...
	        handler: function (widgetColumn) {
	          ...
	          var routingid = record.data.ROUTINGCODE;
	          var params = {
	          ...
	          setTimeout(function () {
	            ...
	            if (count4 == 0) {
	              ...
	              $('#OrderNo').val(record.data.OUTPONO);
	              $('#OrderSeq').val(record.data.OUTPOSEQ);
	              $('#OrderDate').val(Ext.Date.format(record.data.PODATE, 'Y-m-d'));
	              $('#WorkOrderId').val(record.data.WORKORDERID);
	              $('#WorkOrderSeq').val(record.data.WORKORDERSEQ);
	              $('#RoutingName').val(record.data.ROUTINGNAME);
	              ...
	              $('#OrderQty').val(record.data.EXTRANSQTY);
	              $('#CheckQty').val(record.data.EXTRANSQTY);
	              
	              // 예정수량을 넘지않도록 체크하는 부분
	              $('#ExtransQty').val(record.data.EXTRANSQTY);
	  ...
	  items["api.4"] = {};
	  $.extend(items["api.4"], {
	    read: "<c:url value='/ScmOutprocessInspRegistPop.do' />"
	  });
...
// 	  });
	  ...
	  items["api.44"] = {};
	  $.extend(items["api.44"], {
	    read: "<c:url value='/select/scm/outprocess/OutProcessInspPopup.do' />"
	  });
	      ...
	      me.callParent([Ext.apply({
	            ...
	            proxy: {
	              ...
	              extraParams: {
	                ...
	                CUSTOMERCODE: $('#CustomerCode').val(),
	                //                  CUSTOMERCODE: $('#popupCustomerCode').val(),
	                //WORKORDERID: $('#popupWorkOrderId').val(),
	                ITEMCODE: $('#popupItemCode').val(),
	              },
	              reader: gridVals.reader,
	            }
...
// 	    onMypopClick: onMypopClick,
	...
	function onMypopClick(dataview, record, item, index, e, eOpts) {
	  ...
	  $('#popupOrgId').val($('#searchOrgId option:selected').val());
	  $('#popupCompanyId').val($('#searchCompanyId option:selected').val());
	  var ItemCodeVal = $('#popupItemcodeD').val();
	  var OrgIdVal = $('#popupOrgId').val();
	  var CompanyIdVal = $('#popupCompanyId').val();
	...
	function fn_change_qty() {
		var extransqty = $("#ExtransQty").val() * 1;
	  var checkqty = $("#OrderQty").val() * 1;
	  
	  if(extransqty < checkqty){
		  extAlert("입고예정수량이 발주수량을 초과할 수 없습니다.");
		  $("#OrderQty").val(extransqty);
	...
	function fn_save() {
	  ...
	  if (gridcount == 0) {
	    extAlert("[상세 미등록]<br/> 공정검사 상세 데이터가 등록되지 않았습니다.");
	    return false;
	  }

	  if (isNew) {
	    url = "<c:url value='/insert/scm/outprocess/OutProcessInspRegist.do' />";
	    url1 = "<c:url value='/insert/scm/outprocess/OutProcessInspRegistD.do' />";
	    msgGubun = 1;
	  } else {
	    url = "<c:url value='/update/scm/outprocess/OutProcessInspRegist.do' />";
	    url1 = "<c:url value='/update/scm/outprocess/OutProcessInspRegistD.do' />";
	    msgGubun = 2;
	  }
	  ...
	  if (msgGubun == 1) {
	    Ext.MessageBox.confirm('알림', '저장 하시겠습니까?', function (btn) {
	      if (btn == 'yes') {
	              ...
	              if (params.length > 0) {
	                Ext.Ajax.request({
	                  ...
	                  success: function (conn, response, options, eOpts) {
	                    ...
	                    if (dataSuccess > 0) {
	                      //                          setInterval(function () {
	                      go_url("<c:url value='/scm/outprocess/OutProcessInspRegistD.do?no=' />" + inspno + "&org=" + orgid + "&company=" + companyid);
	                      //                          }, 1 * 0.5 * 1000);
	                    }
	                  },
	                  error: ajaxError
	        ...
	        Ext.Msg.alert('외주공정검사 등록', '외주공정검사 등록이 취소되었습니다.');
	    ...
	    Ext.MessageBox.confirm('외주공정검사 등록 변경 알림', '외주공정검사 등록을 변경하시겠습니까?', function (btn) {
	      if (btn == 'yes') {
	...
	function fn_search() {
	  ...
	  url = "<c:url value='/select/scm/outprocess/OutProcessInspMasterRegist.do' />";
	  ...
	  $.ajax({
	    ...
	    success: function (data) {
	      ...
	      var orderseq = dataList.ORDERSEQ;
	      ...
	      var orderqty = dataList.ORDERQTY;
	      ...
	      var workorderid = dataList.WORKORDERID;
	      var workorderseq = dataList.WORKORDERSEQ;
	      var routingname = dataList.ROUTINGNAME;
	      var remarks = dataList.REMARKS;
	      var extransqty = dataList.EXTRANSQTY;
	...
	function fn_print() {
	  ...
	  var column = 'master';
	  var url = "<c:url value='/report/ScmOutCheckSheetReport.pdf'/>";
	  var target = '_blank';
	...
	function fn_delete() {
	  ...
	  url = "<c:url value='/delete/scm/outprocess/OutProcessInspRegistMD.do' />";
	      ...
	      $.ajax({
	        ...
	        success: function (data) {
	          ...
	          if (result) {
	            ...
	            extAlert(msg);
	            setInterval(function () {
	              go_url("<c:url value='/scm/outprocess/OutProcessInspRegist.do' />");
	            }, 1 * 0.5 * 1000);
	          } else {
	            // 실패 했을 경우
	            var msg = data.masage;
                ...
                <div id="container">
                        ...
                        <div id="content">
                                ...
                                <div id="search_field">
                                        ...
                                        <input type="hidden" id="ExtransQty" name="ExtransQty" />
                                        ...
                                        <input type="hidden" id="popupWorkOrderId" name="popupWorkOrderId" /> 
                                        <input type="hidden" id="popupWorkOrderSeq" name="popupWorkOrderSeq" /> 
                                                                        ...
                                                                        <tr style="height: 34px;">
                                                                                ...
                                                                                <td>
                                                                                        <div class="buttons" style="float: right; margin-top: 3px;">
                                                                                            <a id="btnChk1" class="btn_search" href="#" onclick="javascript:btnSel1();"> 입고검사대기 List </a> 
                                                                                            ...
                                                                                            <a id="btnChk6" class="btn_print" href="#" onclick="javascript:fn_print();"> 자주검사 Check Sheet </a>
                                                                                        </div>
                                                                                </td>
                                                                        </tr>
                                                                </table>
...
<%--                                                                             <col width="22%"> --%>
                                                                        ...
                                                                        <tr style="height: 34px;">
                                                                                ...
                                                                                <td>
                                                                                      <input type="text" id="InspNo" name="InspNo" class="input_center" style="width: 97%;" value="${searchVO.INSPECTIONPLANNO}" readonly />
                                                                                </td>
                                                                                <th class="required_text">검사일</th>
                                                                                <td>
                                                                                      <input type="text" id="InspDate" name="InspDate" class="input_validation input_center" style="width: 97%;" maxlength="10"  />
                                                                                </td>
                                                                                <th class="required_text">작업지시번호</th>
                                                                                <td>
                                                                                      <input type="text" id="WorkOrderId" name="WorkOrderId" class="input_center" style="width: 97%;" readonly />
                                                                                      <input type="hidden" id="WorkOrderSeq" name="WorkOrderSeq" class="" />
                                                                                      <input type="hidden" id="OrderNo" name="OrderNo" class=""  />
                                                                                      <input type="hidden" id="OrderSeq" name="OrderSeq" class="" />
                                                                                </td>
<!--                                                                                 <th class="required_text">발주번호</th> -->
<!--                                                                                 <td> -->
<!--                                                                                       <input type="text" id="OrderNo" name="OrderNo" class="input_center" style="width: 97%;" readonly /> -->
<!--                                                                                       <input type="hidden" id="OrderSeq" name="OrderSeq" class="" /> -->
<!--                                                                                 </td> -->
<!--                                                                                 <th class="required_text">발주일</th> -->
<!--                                                                                 <td> -->
<!--                                                                                       <input type="text" id="OrderDate" name="OrderDate" class=" input_center" style="width: 97%;" maxlength="10" readonly /> -->
<!--                                                                                 </td> -->
                                                                        </tr>
                                                                        <tr style="height: 34px;">
                                                                                ...
                                                                                <td>
                                                                                      ...
                                                                                      <input type="hidden" id="RoutingCode" name="RoutingCode" class="" /> 
                                                                                </td>
<!--                                                                                 <th class="required_text">재질</th> -->
<!--                                                                                 <td> -->
<!--                                                                                       <input type="text" id="MaterialType" name="MaterialType" class="input_center" style="width: 97%;" readonly/>  -->
<!--                                                                                 </td> -->
                                                                        </tr>
                                                                        <tr style="height: 34px;">
                                                                                ...
                                                                                <th class="required_text">생산수량</th>
                                                                                <td>
                                                                                      <input type="text" id="OrderQty" name="OrderQty" class="input_validation input_center" style="width: 97%;" onchange="javascript:fn_change_qty();"/> 
                                                                                </td>
                                                                        ...
                                                                        <tr style="height: 34px;">
                                                                                ...
                                                                                <td>
                                                                                      <input type="text" id="PassQty" name="PassQty" class="input_center" style="width: 97%;" readonly/> 
                                                                                </td>
                                                                                <th class="required_text">불합격수량</th>
                                                                                <td>
                                                                                      <input type="text" id="FailQty" name="FailQty" class="input_center" style="width: 97%;" readonly/> 
                                                                                </td>
                                                                                <th class="required_text">결과</th>
                                                                                <td>
                                                                                      <input type="text" id="CheckYnName" name="CheckYnName" class="input_validation input_center" style="width: 97%; margin-top: 6px;" onkeydown="javascript:fn_check_result2();" onkeyup="javascript:fn_check_result2();" /> 
                                                                        ...
                                                                        <tr style="height: 34px;">
                                                                                ...
                                                                                <td colspan="5">
                                                                                      <input type="text" id="Remarks" name="Remarks" class="input_left" style="width: 99%;" />
                                                                                </td>
                                                                        </tr>
                                                                </table>
                                                        </div>
                ...
                <!-- //container 끝 -->
                <div id="gridPopup1Area" style="width: 1305px; padding-top: 0px; float: left;"></div>
