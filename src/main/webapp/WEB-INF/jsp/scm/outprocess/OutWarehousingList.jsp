...
$(document).ready(function () {
	...
	function setValues() {
	  ...
	  gridnms["grid.1"] = "OutWarehousingMaster";
	  gridnms["grid.2"] = "OutWarehousingDetail";
	  ...
	  fields["columns.1"] = [
	    // Display Columns
	    {
	      ...
	      renderer: function (value, metaData, record, rowIndex, colIndex, store, view) {
	        ...
	        var url = "<c:url value='/scm/outprocess/OutWarehousingRegistDetail.do?no=' />" + record.data.OUTTRANSNO + "&org=" + record.data.ORGID + "&company=" + record.data.COMPANYID;
	  ...
	  items["api.1"] = {};
	  $.extend(items["api.1"], {
	    read: "<c:url value='/select/scm/outprocess/OutWarehousingList.do' />"
	  });

	  items["api.2"] = {};
	  $.extend(items["api.2"], {
	    read: "<c:url value='/select/scm/outprocess/OutWarehousingGrid.do' />"
	  });
	...
	function onMyviewItemcodelistClick(dataview, record, item, index, e, eOpts) {
	  ...
	  $("#no").val(record.get("OUTTRANSNO"));
	  $("#orgid").val(record.get("ORGID"));
	  $("#customercode").val(record.get("CUSTOMERCODE"));
	...
	function fn_search() {
	  ...
	  var transfrom = $('#searchWorkOrderFrom').val();
	  var transto = $('#searchWorkOrderTo').val();
	  var header = [],
	  count = 0;
...
// 	    WORKORDERID: $('#searchWorkOrderNo').val(),
	...
	function setLovList() {
	  ...
	  // 작업지시번호 Lov
	  $("#searchWorkOrderNo").bind("keydown", function (e) {
	    switch (e.keyCode) {
	    case $.ui.keyCode.TAB:
	    ...
	    source: function (request, response) {
	      $.getJSON("<c:url value='/searchWorkNoListLov.do' />", {
	        ...
	        WORKPLANDATE: $('#searchWorkOrderFrom').val(),
	        //WORKDATE: $('#searchWorkOrderFrom').val(),
	    ...
	    select: function (e, o) {
	      $("#searchWorkOrderNo").val(o.item.value);
...
// 	          keyword: extractLast(request.term),
// 	          OUTTRANSFROM: $('#searchWorkOrderFrom').val(),
// 	          OUTTRANSTO: $('#searchWorkOrderTo').val(),
// 	          ORGID: $('#searchOrgId option:selected').val(),
// 	          COMPANYID: $('#searchCompanyId option:selected').val(),
// 	          //            GUBUN: 'LIST',
// 	        }, function (data) {
                            ...
                            <div>
                                ...
                                <table class="tbl_type_view" border="1">
                                    ...
                                    <tr style="height: 34px;">
                                        ...
                                        <td >
                                            <input type="text" id="searchWorkOrderFrom" name="searchWorkOrderFrom" class="input_validation input_center validate[custom[date],past[#searchWorkOrderTo]]" style="width: 90px; " maxlength="10" />
                                            &nbsp;~&nbsp;
                                            <input type="text" id="searchWorkOrderTo" name="searchWorkOrderTo" class="input_validation input_center validate[custom[date],future[#searchWorkOrderFrom]]" style="width: 90px; " maxlength="10"  />
                                        </td>
<!--                                         <th class="required_text">작업지시번호</th> -->
<!--                                         <td> -->
<!--                                             <input type="text" id="searchWorkOrderNo" name="searchWorkOrderNo" class="input_left"  style="width: 97%;" /> -->
<!--                                         </td>                                         -->
...
<!--                                             <input type="hidden" id="searchItemCode" name="searchItemCode" class=""  /> -->
                                        ...
                                        <th class="required_text">출고상태</th>
                                        <td >
<!--                                             <input type="text" id="searchStatus" name="searchStatus" class="input_left"  style="width: 97%;" /> -->
                                            <select id="searchStatus" name="searchStatus" class="input_left validate[required]" style="width: 97%;">
                                                <c:if test="${empty searchVO.TRANSYN}">
                                                    <option value="" label="전체" />
                                                </c:if>
                                                <c:forEach var="item" items="${labelBox.findByReleaseLov}" varStatus="status">
                                                    <c:choose>
                                                        <c:when test="${item.VALUE==searchVO.TRANSYN}">
                                                            <option value="${item.VALUE}" selected>${item.LABEL}</option>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <option value="${item.VALUE}">${item.LABEL}</option>
