...
$(document).ready(function () {
	...
	function setInitial() {
	  ...
	  // 입하일자
	  calender($('#searchLotFrom, #searchLotTo'));

	  $('#searchLotFrom, #searchLotTo').keyup(function (event) {
	    if (event.keyCode != '8') {
	      var v = this.value;
	      if (v.length === 4) {
	        this.value = v + "-";
	  ...
	  $("#searchLotFrom").val(getToDay("${searchVO.dateFrom}") + "");
	  $("#searchLotTo").val(getToDay("${searchVO.dateTo}") + "");
	...
	function setValues() {
	  ...
	  fields["columns.1"] = [
	    // Display Columns
	    {
	      ...
	      renderer: function (value, metaData, record, rowIndex, colIndex, store, view) {
	        ...
	        var url = "<c:url value='/scm/outprocess/OutProcLotRegistD.do?no=' />" + record.data.LOTNO + "&org=" + record.data.ORGID + "&company=" + record.data.COMPANYID;
	  ...
	  items["api.1"] = {};
	  $.extend(items["api.1"], {
	    read: "<c:url value='/select/scm/outprocess/OutProcLotRegist.do' />"
	  });
	      ...
	      me.callParent([Ext.apply({
	            ...
	            proxy: {
	              ...
	              extraParams: {
	                ...
	                LOTFROM: '${searchVO.dateFrom}',
	                LOTTO: '${searchVO.dateTo}',
	  ...
	  Ext.define(gridnms["store.2"], {
	    ...
	    constructor: function (cfg) {
	      ...
	      me.callParent([Ext.apply({
	            ...
	            clearOnPageLoad: true,
	            clearRemovedOnLoad: true,
	            pageSize: 9999, // gridVals.pageSize,
	            proxy: {
	              type: 'ajax',
	              api: items["api.2"],
	              extraParams: {
	          ...
	          listeners: {
	            refresh: function (dataView) {
	              Ext.each(dataView.panel.columns, function (column) {
	                if (column.dataIndex.indexOf('ORDERNAME') >= 0 || column.dataIndex.indexOf('ITEMNAME') >= 0 || column.dataIndex.indexOf('CUSTOMERNAME') >= 0) {
	                  column.autoSize();
	                  column.width += 5;
	                  if (column.width < 80) {
	                    column.width = 80;
	...
	function fn_search() {
	  ...
	  var sparams = {
	    ...
	    LOTFROM: $('#searchLotFrom').val(),
	    LOTTO: $('#searchLotTo').val(),
	...
	function setLovList() {
	  ...
	  // lot번호 Lov
	  $("#searchLotNo").bind("keydown", function (e) {
	    switch (e.keyCode) {
	    case $.ui.keyCode.TAB:
	      if ($(this).autocomplete("instance").menu.active) {
	        e.preventDefault();
	    ...
	    source: function (request, response) {
	      $.getJSON("<c:url value='/scm/mat/MatLotBarcodeRegistLotNoLovList.do' />", {
	        keyword: extractLast(request.term),
	        ORGID: $('#searchOrgId option:selected').val(),
	        COMPANYID: $('#searchCompanyId option:selected').val(),
	        LOTNO: $('#searchLotNo').val(),
        ...
        <div id="container">
            ...
            <div id="content">
                    ...
                    <div id="search_field">
                        ...
                        <input type="hidden" id="lotno" value="<c:out value='${LotNoVal}'/>" />
                            ...
                            <div>
                                ...
                                <table class="tbl_type_view" border="1">
                                    ...
                                    <tr style="height: 34px;">
                                        ...
                                        <td >
                                            <input type="text" id="searchLotFrom" name="searchLotFrom" class="input_validation input_center validate[custom[date],past[#searchLotTo]]" style="width: 90px; " maxlength="10" />
                                            &nbsp;~&nbsp;
                                            <input type="text" id="searchLotTo" name="searchLotTo" class="input_validation input_center validate[custom[date],future[#searchLotFrom]]" style="width: 90px; " maxlength="10"  />
                                        </td>
                                        <th class="required_text">LOT 번호</th>
                                        <td >
                                            <input type="text" id="searchLotNo" name="searchLotNo" class="input_center"  style="width: 97%;" />
                                        </td>
