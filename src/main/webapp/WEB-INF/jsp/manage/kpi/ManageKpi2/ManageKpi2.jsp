...
google.charts.load('current', {
	...
	var status_data;
	var header_g;
	//  var header_g = [' ', 'JIG조립', '열건스핀들', 'UV스핀들', 'XY라인', 'XY해체', 'XY조립', '로봇JIG조립', '로봇도장', '로봇중간조립', '로봇해체', '로봇해체/조립', '수동라인', '수동조립', '수동해체', '수동해체/조립', '증착기', '인쇄기', '레이져', '해체실', '중간검사', '최종검사', '광주검사', '중간외주', '최종외주'];
	...
	function drawVisualization() {
	  var sparams = {
	    ...
	    WORKCNETERCODE: $('#searchItemnm2').val(),
	  };

	  var datayear = $("#searchdateYear").val();

	  var url_x = '<c:url value="/select/manage/kpi/ManageKpi2List2.do" />';
	  $.ajax({
	    url: url_x,
	    type: "post",
	    dataType: "json",
	    ...
	    success: function (data) {
	      ...
	      var prodname_temp = new Array();
	      var prodname_title = new Array();
	      var proddate_temp = new Array();
	      ...
	      for (var i = 0; i < charcount; i++) {
	        ...
	        count = 0;
	        if (i == 0) {
	          prodname_temp[prodname_temp.length] = list.WORKCENTERNAME;
	        } else {
	          for (var j = 0; j < prodname_temp.length; j++) {
	            if (prodname_temp[j] == list.WORKCENTERNAME) {
	              // 같은 경우 카운트
	              ++count;
	            }
	          }
	      ...
	      for (var n = 0; n < 12; n++) {
	        ...
	        prodname_title[0] = " ";
	        if (n < 9) {
	          prodqty_temp[0] = "0" + (n + 1) + "월";
	        } else {
	          prodqty_temp[0] = (n + 1) + "월";
	        }
	        for (var k = 0; k < prodname_temp.length; k++) {
	          v_count = 0;
	          var qty = 0;
	          // 중복제거된 설비명 Loop
	          for (var i = 0; i < charcount; i++) {
	            ...
	            if (prodname_temp[k] == list.WORKCENTERNAME) { // 이름이 중복 된 애들의 데이터를 넣지않기위해서 설비명 중복여부 체크

	              if ((list.WORKENDDATE * 1) == (n + 1)) { // 같은 일자인지 체크
	                //                      prodqty_temp.push( list.PRODUCEDQTY );
	                //                      prodname_temp.push( list.WORKCENTERNAME );
	                ++v_count;
	                qty = list.PRODUCEDQTY;
	              }
	            }
	          ...
	          if (v_count > 0) {
	            ...
	            prodname_title.push(prodname_temp[k]);
	...
	function setLineChart(data_val) {
	  ...
	  // formatter.format(data_val, 1);
	  //    formatter.format(data_val, 2);
	  //    formatter.format(data_val, 3);
	  //    formatter.format(data_val, 4);
	...
	google.setOnLoadCallback(drawCharter);
	...
	function drawCharter() {
	  var sparams = {
	    ...
	    WORKCNETERCODE: $('#searchItemnm').val(),
	    ROUTINGCODE: $('#searchRoutingnm').val(),
	  };

	  var url_x = '<c:url value="/select/manage/kpi/ManageKpi2List3.do" />';
	  $.ajax({
	    url: url_x,
	    type: "post",
	    dataType: "json",
	    ...
	    success: function (data) {
	      ...
	      for (var i = 0; i < charcount; i++) {
	        ...
	        for (k = 0; k < work_hd.length; k++) {
	          if (work_hd[k] == list.WORKCENTERNAME) {
	            n_count++;
	          }
	        }
	        if (n_count == 0) {
	          work_hd.push(list.WORKCENTERNAME);
	        }
	      }
	      work_hd.sort();
	      ...
	      for (var j = 0; j < charcount; j++) {
	        ...
	        for (l = 0; l < date_temp.length; l++) {
	          if (date_temp[l] == list.WORKENDDATE) {
	            c_count++;
	          }
	        }
	        if (c_count == 0) {
	          date_temp.push(list.WORKENDDATE);
	        }
	      }
	      date_temp.sort();
	      ...
	      for (n = 0; n < date_temp.length; n++) {
	        ...
	        for (var k = 0; k < work_hd.length; k++) {
	          ...
	          for (var i = 0; i < charcount; i++) {
	            ...
	            if (work_hd[k] == list.WORKCENTERNAME) { // 이름이 중복 된 애들의 데이터를 넣지않기위해서 설비명 중복여부 체크

	              if ((list.WORKENDDATE * 1) == enddate) { // 같은 일자인지 체크
	                //                                            prodqty_temp.push( list.PRODUCEDQTY );
	                //                                            prodname_temp.push( list.WORKCENTERNAME );
	                ++v_count;
	                qty = list.WORKOFTIME;
	...
	function setValues() {
	  ...
	  items["api.1"] = {};
	  $.extend(items["api.1"], {
	    read: "<c:url value='/select/manage/kpi/ManageKpi2List1.do' />"
	  });
...
//	         extGridSearch(params, gridnms["store.2"]);
	...
	function fn_search() {
	  ...
	  url = "<c:url value='/select/manage/kpi/ManageKpi2List3.do' />";
	  ...
	  $.ajax({
	    ...
	    success: function (data) {
	      ...
	      if (data.totcnt == 0) {
	        extAlert("해당 제조 월의 설비 데이터가 없습니다.");
	        drawCharter();
	      } else {
	        drawCharter();
	      }
	...
	function fn_search_y() {
	  ...
	  url = "<c:url value='/select/manage/kpi/ManageKpi2List2.do' />";
...
<!--                    <input type="text" id="searchTo" name="searchTo" class="input_validation input_center" style="width: 90px;" maxlength="10" /> -->
                    ...
                    <td><select id="searchItemnm2" name="searchItemnm2" class="input_left validate[required]" style="width: 150px;">
                ...
                <div id="chart_div" style="width: 100%; height: 30%"></div>
                    ...
                    <td><select id="searchRoutingnm" name="searchRoutingnm" class="input_left validate[required]" style="width: 150px;">
                        <c:if test="${empty searchVO.RoutingCode}">
                            <option value="" label="전체" />
                        </c:if>
                        <c:forEach var="item" items="${labelBox.findByRoutingCode}" varStatus="status">
                            <c:choose>
                                <c:when test="${item.VALUE==searchVO.RoutingCode}">
                                    <option value="${item.VALUE}" selected>${item.LABEL}</option>
                                </c:when>
                                <c:otherwise>
                                    <option value="${item.VALUE}">${item.LABEL}</option>
				...
				<!-- //검색 필드 박스 끝 -->
				<div style="width: 100%;">
					<div id="gridViewArea" style="width: 77%; padding-bottom: 5px; float: left;"></div>
				</div>
			</div>
			<!-- //content 끝 -->
		</div>
