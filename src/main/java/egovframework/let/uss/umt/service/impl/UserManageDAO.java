...
public class UserManageDAO extends EgovAbstractDAO{
    ...
    public String insertUser(UserManageVO userManageVO){
    	// 사용자 아이디 자동 생성부분 주석 처리
    	/*if( !userManageVO.getGroupId().equals("GROUP_00000000000000") ){
    		System.out.println("========================userManageDAO.userSeq==============");
    		String seq = (String)select("userManageDAO.userSeq", userManageVO);
    		if(userManageVO.getGroupId().equals("GROUP_00000000000001")){
    			userManageVO.setEmplyrId("s"+seq);
    		
    		}else if(userManageVO.getGroupId().equals("GROUP_00000000000003")){
    			userManageVO.setEmplyrId("c"+seq);
    		}else if(userManageVO.getGroupId().equals("GROUP_00000000000002")){
    			userManageVO.setEmplyrId("f"+seq);
    		}else{
    			userManageVO.setEmplyrId("f"+seq);
    		}
    		
    	}*/
    	//System.out.println("aaa " + userManageVO.getEmplyrId());
     ...
     */
    @SuppressWarnings("unchecked")
	public List selectErpUserListPop(UserDefaultVO userSearchVO){
        return list("userManageDAO.selectErpUserList_S", userSearchVO);
    }
     ...
     * @return int 업무사용자 총갯수
     */
    public int selectErpUserListPopTotCnt(UserDefaultVO userSearchVO) {
        return (Integer)select("userManageDAO.selectErpUserListTotCnt_S", userSearchVO);
    }
    ...
    @SuppressWarnings("unchecked")
	public List selectErpUserListPop2(UserDefaultVO userSearchVO){
        return list("userManageDAO.selectErpUserList2_S", userSearchVO);
    }
    
   
    public int selectErpUserListPop2TotCnt(UserDefaultVO userSearchVO) {
        return (Integer)select("userManageDAO.selectErpUserList2TotCnt_S", userSearchVO);
    }
    
    
    @SuppressWarnings("unchecked")
	public List selectCarUserListPop(UserDefaultVO userSearchVO){
    	
        return list("userManageDAO.selectCarUserList_S", userSearchVO);
    }
    
    public int selectCarUserListPopTotCnt(UserDefaultVO userSearchVO) {
        return (Integer)select("userManageDAO.selectCarUserListTotCnt_S", userSearchVO);
    }
    ...
    public void sessionAdd(){
    	update("returnWarehouseDAO.alterSession", null);
    }
}